...
those details from recorded calls. Target price: $4/month.
...
- **Settings** — stores API keys locally (Claude, OpenAI, bot server URL+token,
  Recall.ai fallback), profile, Free vs $4/mo Pro plan UI.
...
- Real Stripe payments for the $4/mo Pro plan (currently UI only).
