// Synced follow-up reminders. Same scoping/merge approach as clients & events.
...
router.get('/', requireAuth, (req, res) => {
  ...
  let sql = `SELECT id, data_json, updated_at, deleted FROM reminders WHERE ${where}`;
  const args = [...params];
  if (since) { sql += ' AND updated_at > ?'; args.push(since); }
  const rows = db.prepare(sql).all(...args);
  res.json({
    reminders: rows.map(r => ({ id: r.id, deleted: !!r.deleted, data: r.deleted ? null : JSON.parse(r.data_json), updatedAt: r.updated_at })),
...
router.put('/:id', requireAuth, (req, res) => {
  ...
  if (existing) {
    ...
    db.prepare('UPDATE reminders SET data_json = ?, updated_at = ?, deleted = 0 WHERE id = ?').run(JSON.stringify(data), ts, req.params.id);
  } else {
    db.prepare('INSERT INTO reminders (id, owner_id, org_id, data_json, updated_at, deleted) VALUES (?, ?, ?, ?, ?, 0)')
...
router.delete('/:id', requireAuth, (req, res) => {
  ...
  const inScope = req.user.org_id ? existing.org_id === req.user.org_id : existing.owner_id === req.user.id;
  if (!inScope) return res.status(403).json({ error: 'Not allowed' });
  db.prepare('UPDATE reminders SET deleted = 1, updated_at = ? WHERE id = ?').run(new Date().toISOString(), req.params.id);
  res.json({ ok: true });
});
